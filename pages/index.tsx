import type { NextPage, GetServerSideProps } from "next";
import Head from "next/head";
import { parseCookies, setCookie, destroyCookie } from "nookies";
import Clipboard from 'react-clipboard.js';
import { LocationMarkerIcon, PhoneIcon } from "@heroicons/react/solid";
import Image from "next/image";
import { useEffect, useState } from "react";
import { confirmPresence, addGuest } from "../services/firebase";
import Router from "next/router";

const Home: NextPage = () => {
  const [phone, setPhone] = useState(String);
  const [err, setErr] = useState(String);
  const handlePhone = (number: string) => {
    const regex = new RegExp("^[0-9]*$");

    if (regex.test(number) !== false && number.length < 10) {
      setPhone(number);
    } else {
    }
  };
  function verifyCookie() {
    const { "convite-digital": cookie } = parseCookies();
    if (cookie) {
      Router.push("/confirmed");
    }
  }
  useEffect(() => {
    verifyCookie();
  }, []);

  const handleConfirmation = async () => {
    if (!phone) {
      return setErr(
        "Insira o seu primeiro nome sem acentuações (ex: ´, ^, ~)."
      );
    }
    const result = await confirmPresence(phone, "confirm");

    if (result) {
      alert("Presença confirmada");
      setCookie(null, "convite-digital", phone, {
        maxAge: 30 * 24 * 60 * 60,
        path: "/",
      });
      Router.push("/confirmed");
    } else {
      return setErr("Infelizmente o seu nome não consta na lista ou tente inserir o seu primeiro nome sem acentuações (ex: ´, ^, ~).");
    }
  };

  const location = {
    lat: "-8.9633218",
    long: "13.2020731",
  };

  const mapMapUrl = `https://www.google.com/maps/search/?api=1&query=${location.lat}%2C${location.long}`;
  
  return (
    <div className= " flex flex-col items-center justify-center min-h-screen text-white bg-center bg-cover bg-bg main-container ">
      <Head>
        <title>Convite digital</title>
        <link rel="icon" href="/tapago.svg" />
      </Head>
       
      <section className="flex justify-center flex-col flex-1 w-full max-w-100 l-451 ">

      
        <div className="flex justify-center flex-row mt-[80px] ">
          <img alt="Flores à direita do nome" src="/Flowers_left.png" className="w-24 h-44 ml-[-20px]"></img>
          <h1 className="z-10 sm:mt-[35px] md:mt-[55px] italic text-[31px] text-center font-lucida_calligraphy  ">Cíntia Alexandre</h1>
          <img alt="Flores à esquerda do nome" src="/Flowers_right.png" className="w-24 h-37 pr-[-10px]"></img>
        </div>

        <div className="flex flex-col mt-[-55px] text-center align-middle">
        <p className="text-[96px] align-center font-lucida_bright">18</p>
        </div>

        <div className="flex flex-col mt-0 space-y-2 text-center align-middle">
          <p className="text-[22px] font-lucida_calligraphy">Birthday</p>
          <p className="text-md font-lucida_bright">31/06</p>
          <p className="font-lucida_bright text-md">18:00</p>
        </div>

        <div className="flex flex-row mt-3 text-center  justify-center align-middle">
        <p className="text-[90px] ml-[-10px] font-lucida_calligraphy mr-0">“</p>
        <p className="pt-6 ml-[-4px] w-[250px] text-b font-lucida_bright text-center underline-offset-2">
        Recarregue a sua energia e venha celebrar comigo!
          </p>
        </div>
        </section>
      
      <main className="flex flex-col justify-start w-full max-w-md px-5 text-center">
        <div className="flex flex-col mt-10">
          <label htmlFor="phone" className="text-left font-lucida_bright text-white text-md">
            Primeiro nome: 
          </label>
          <input
            className=" font-lucida_bright p-0 py-1 pt-2 text-white bg-transparent border-b border-white border-x-transparent border-t-transparent placeholder:text-gray-300"
            type="tel"
            onFocus={() => setErr("")}
            onChange={(e) => handlePhone(e.target.value)}
            placeholder="Por favor insira o seu primeiro nome"
            value={phone}
          />
          <span className="mt-3 text-left text-red-400 text-md">
            {err && `*${err}`}
          </span>
          <div
            onClick={handleConfirmation}
            className="flex font-lucida_bright justify-center flex-1 p-5 mx-auto mt-6 align-middle transition-all border-2 border-white cursor-pointer lg:w-full hover:bg-white hover:text-black"
          >
            Confirmar Presença
          </div>
        </div>

        <div>
          <div className="flex flex-col mt-16 ">
          <img alt="Localização" src="https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/160/apple/114/round-pushpin_1f4cd.png" className="self-center block w-6 h-6"></img>
            <p className="relative font-lucida_bright justify-center mt-5">
            Rua P, frente ao Condomínio Kuditemo.
            </p>
            <a
                className="self-center mt-1 ml-1 "
                href={mapMapUrl}
              >
                {" "}
                <p className="inline font-lucida_bright font-medium text-white border-b border-white ottom-0">Ver no mapa</p>
                <img alt="" src="data:image/svg+xml;base64," className="inline w-6 h-6 ml-2"></img>
              </a>
          </div>

          <div className="flex flex-col mt-16 ">
          <img alt="Contacto" src="https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/160/apple/21/black-telephone_260e.png" className="self-center block w-6 h-6"></img>
            <p className="relative font-lucida_bright justify-center mt-5">
            +244 923492663
            </p>
            <Clipboard data-clipboard-text="+244 923492663">
        <p className="inline font-lucida_bright font-medium text-white border-b border-white ottom-0">Copiar</p>
        <img alt="" src="data:image/svg+xml;base64," className="inline w-5 h-5 ml-2"></img>
      </Clipboard>
          </div>
          
          <div className="bottom-0 left-0 right-0 flex items-center justify-center mt-16">
            <img alt="" src=""></img>
          </div>
        </div>
      </main>
    </div>
  );
};

export default Home;
